...
class Producer(object):
	...
	def __init__(self):
		...
		## Index Initializers
		self.INDEX_DICTIONARY = self.hv.INDEX_DICTIONARY_INITIALIZER



		print("Before Kafka_connection###################################")
	...
	def index_incrementor(self,peopleid,index):
		...
		if(index > self.hv.TOTAL_DICTIONARY[peopleid]):
	...
	def keep_rolling(self):
		...
		map(lambda x,y: self.rolling_window(x,y), self.hv.SUBJECT_LIST_ID, self.INDEX_DICTIONARY.values())
	...
	def start_rolling(self):
		...
		while self.flag:
			...
			self.keep_rolling()
			if (self.iteration_index >= self.hv.TOTAL_ITERATIONS):
				self.flag = False

			self.iteration_index += 1
			time.sleep(self.hv.TIME_DURATION_BETWEEN_SUCCESSIVE_SEQNO)
